...
- Consensus
header:
  teaser: "/images/hotstuff-machine.jpeg"
  overlay_image: "/images/hotstuff-machine.jpeg"
...
In this post, we describe the construction of **VABA**,
by Abraham, Malkhi et al. 
...
The solution makes a direct use of [HotStuff](Yin2019HotStuffBC) and moreover, follows the two-part decomposition described in a [previous post](https://malkhi.com/posts/2025/01/hs-from-vpcbc/):
...
To prevent an asynchronous adversary from block every leader in its turn, Validators **elect a leader at random** via a distributed protocol to drive one
view of HotStuff. 
...
it operates $n$ parallel-worlds of HotStuff, each with a different
...
VABA borrows the protocol template of Consensus solutions in 
the [Partial Synchrony model](https://malkhi.com/posts/2025/01/models/).
It operates in a view-by-view manner, each view operated by a leader. 
If the leader is honest, then the view is successful
and the protocol terminates.

Instead of a single view, VABA operates $n$ parallel ones, referred to as an *iteration*.
Each iteration proceeds as three sub-procedures, depicted below.
