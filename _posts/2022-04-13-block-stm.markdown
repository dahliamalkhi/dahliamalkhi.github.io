...
via two synchronization counters, one per task type, `nextPrelimExecution` (initially 1) and `nextValidation` (initially 2). In order to coordinate teask stealing, each synchronizer `x` supports an atomic `x.increment() { oldVal := x; increment x; return oldVal }`. 

Each transaction (re-)execution is followed an atomic `nextValidation.setMin(val) { nextValidation := min(val, nextValidation) }`, 
