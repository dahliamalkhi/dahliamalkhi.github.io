...
via two synchronization counters, one per task type, `nextPrelimExecution` (initially 1) and `nextValidation` (initially 2). In order to coordinate task stealing, each synchronizer `x` supports an atomic `x.increment() { oldVal := x; increment x; return oldVal }`. 

Each (re-)execution of TX<sub>j</sub> is followed by an atomic `nextValidation.setMin(j+1) { nextValidation := min(j+1, nextValidation) }`, 
guaranteeing that validation of all higher transactions will be dispatched.
