...
<pre style="font-size: 14px;">
    ...
    repeat {
        ...
        atomic {
            if nextValidation < nextPrelimExecution {   // if available, steal next read-set validation task
                (task, j) := ("validate", nextValidation)
                nextValidation.increment();
            } else if nextPrelimExecution <= n {        // if available, steal next execution task
