...
<pre style="font-size: 14px;">
    ...
    execution of TX<sub>j</sub>:  
    {
        abortedWS := TX<sub>j</sub>'s previously ABORTED write-set (empty if none)
        (re-)execute TX<sub>j</sub> and resume any TX suspended on abortedWS
        if TX<sub>j</sub>'s new write-set contains locations not in abortedWS
            nextValidation.setMin(j+1) 
    }
</pre>
