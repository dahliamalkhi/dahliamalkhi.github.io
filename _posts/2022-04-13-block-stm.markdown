...
<pre style="font-size: 14px;">
    ...
    {
        (re-)execute TX<sub>j</sub>
        abortWS := TX<sub>j</sub>'s previously ABORTED write-set 
        if there exists a TX<sub>k</sub> suspended on abortWS
            resume TX<sub>k</sub>
        if TX<sub>j</sub>'s new write-set contains locations not in abortWS
            nextValidation.setMin(j+1) 
    }
</pre>
