...
Each transaction (re-)execution is followed by an atomic `nextValidation.setMin(val) { nextValidation := min(val, nextValidation) }`, 
guaranteeing validation of all higher transactions.
