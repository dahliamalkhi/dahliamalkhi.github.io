...
<pre style="font-size: 14px;">
    ...
    {
        (re-)execute TX<sub>j</sub>
        if TX<sub>k</sub> has been suspended on TX<sub>j</sub>'s previously ABORTED write-set, resume TX<sub>k</sub>
        if the new write-set contains locations not marked ABORTED
            nextValidation.setMin(j+1) 
    }
</pre>
