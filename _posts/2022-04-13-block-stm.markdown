...
<pre style="font-size: 14px;">
    ...
    execution of TX<sub>j</sub>:  
    {
        (re-)execute TX<sub>j</sub> and resume any TX suspended on TX<sub>j</sub> ABORTED write-set

        if TX<sub>j</sub>'s new write-set contains locations not previously marked ABORTED
            nextValidation.setMin(j+1) 
    }
</pre>
