...
Bullshark is a "zero message overhead" protocol over the DAG. However, due to a rigid wave-by-wave structure, 
although Narwhal keeps spreading transactions, it waits for timers that are internal to the Consensus protocol to insert them into the DAG.
...
| Protocol | Model | External messages used | DAG must be layered | **DAG broadcasts wait for Consensus** | Commit latency in DAG broadcasts during synchrony | 
| :--- | :--- | :--- | :--- | :-- | :--- |
...
delay DAG broadcasts according to Consensus protocol round timers, 
