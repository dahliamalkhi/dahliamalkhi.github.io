---
title: 'From Quorum Certified Broadcast (QC) to HotStuff and HotStuff-2'
...
# From Quorum Certified Broadcast (QC) to HotStuff and HotStuff-2
...
First, a leader uses *Quorum Certified Broadcast* [[QC]](https://malkhi.com/posts/2025/01/vpcbc/) to post a new proposal. If it commits, it is guaranteed that $n-f$ Validators hold a lock on it. 
...
The External Validity condition V() employed within Quorum Certified Broadcast verifies that the leader proposal proposal extends the known lock or a higher one. 
...
Since QC incurs only linear Communication complexity, the crux is linearizing the leader handover regime. The goal of a leader handover is to guarantee that Validators accept leader proposals, in order to maintain progress.
...
HotStuff pioneered a different approach, which allowed a linear implementation in the Partial Synchrony setting by utilizing 3-phase QC.  The gist of the new approach is encapsulated by the following handover rule:
...
We dedicate a separate post to the [Handover rule](http://malkhi.com/posts/2025/01/handover/) and show how it can be materialized with linear word-Communication with either 2-phase or 3-phase QC under different setting.   
Note that the difference between HotStuff and HotStuff-2 is simply whether 2 or 3 phases are used in QC, but whether 2 or 3 phases are used matters a lot in materializing the Handover rule. 
