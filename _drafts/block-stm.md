...
* **VALID(j, k)**: When a j-transaction executes (or re-executes), every k-transaction with index k > j (re)validates after the j-transaction completes execution. Validation re-reads the read-set of the k-transaction and compares against the original read-set the k-transaction obtained in its latest execution. If validation fails, the k-transaction re-executes.

No matter what scheduling policy is used, so long as execution and validation tasks are eventually dispatched,
**MVCC** and **VALID(j, k)** jointly suffice to guarantee safety and liveness. For example, a crude strawman scheduling strategy simply validates all transactions in parallel repeatedly until there are no validation failures/re-executions. While this is not the most optimal scheduling strategy, it guarantees **VALID(j, k)**, and that's all that matters for safety/liveness. Safety follows directly from **VALID(j, k)** because a k-transaction gets validated after all j-transactions, j &lt; k, are finalized. Liveness follows by induction. Initially transaction 1 is guaranteed to pass validation successfully and not require re-execution. Once transactions 1..j have successfully validated, the next invocation of transaction j+1 will pass validation successfully and not require re-execution.
